...
public class MercadoPagoServiceImpl implements IMercadoPagoService {
	...
	public MercadoPagoPreferenceResponseDTO crearPreferencia(Pedido pedido) {
		...
		try {
			JsonNode response = RestClient.create(properties.getBaseUrl())
					.post()
	...
	private Map<String, Object> crearRequest(Pedido pedido, Pago pago) {
		...
		if (StringUtils.hasText(properties.getNotificationUrl())) {
			request.put("notification_url", properties.getNotificationUrl());
		}
		if (StringUtils.hasText(properties.getSuccessUrl())
				&& StringUtils.hasText(properties.getFailureUrl())
				&& StringUtils.hasText(properties.getPendingUrl())) {
